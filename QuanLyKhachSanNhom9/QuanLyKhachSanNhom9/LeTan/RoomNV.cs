...
{
    public partial class RoomNV : Form
    {

        //Tạo sự kiện khi ấn vào nút chuyển trang gọi tới sự kiện của form mainNhanVien
        public delegate void SuKienChuyenTrang2(object sender, EventArgs e);
        public SuKienChuyenTrang2 suKienChuyenTrang2;

        public delegate void SuKienCusChiTiet(object sender, EventArgs e);
        public SuKienCusChiTiet suKienCusChiTiet;

        //Tạo sự kiện thêm thông tin và thêm dịch vụ 
        public delegate void SuKienThemDichVu(object sender, EventArgs e);
        public SuKienThemDichVu suKienThemDichVu;

        //Tạo sự kiện tính tiền
        public delegate void SukienTinhTien(object sender, EventArgs e);
        public SukienTinhTien suKienTinhTien;

        //////////////////////////////////////////
        /////Lưu thông tin khi chuyển form
        public static string tenKhachHang = "";
        public static string ID_KhachHang = "";
        public static string MaPhong = "";
        public static int _TongTien = 0;
       ...
       public static string  chuyenTenPhong(string loaiPhong)
        {
            if (loaiPhong.Contains("DoiThuong"))
            {
                return "Đôi Thường";
            }
            else if (loaiPhong.Contains("DoiVip"))
            {
                return "Đôi Vip";
            }
            else if (loaiPhong.Contains("DonThuong"))
                return "Đơn Thường";
            else if (loaiPhong == "DonVip")
            {
                return "Đơn Vip";
            }
            else if (loaiPhong.Contains("GDThuong"))
            {
                return "Gia đình Thường";
            }
            else return "Gia đình Vip";  
        }
        public RoomNV()
        {
            InitializeComponent();
        ...
        private void loadRoomStart(object sender, EventArgs e)//load 12 dòng đầu tiên để test thử
        {
            using (SqlConnection sqlCon = new SqlConnection(TruyVan.strConnection))
            {
                string query = $@"
                        SELECT 
                            ...
                            NULL AS ThoiGian
                        FROM phong AS p
                        WHERE p.TrangThai = N'Trống' OR p.TrangThai = N'Sửa Chữa'
                        UNION
                        SELECT 
                            p.ID_Phong,
                            p.ID_LoaiPhong,
                        ...
                        WHERE p.TrangThai = N'Có Người' AND k.TimeCheckOut IS NULL;           
                                ";
                //tạo bảng có ba cột phòng thêm một cột nếu trạng thái phòng là N'Có Người' thì kiểm tra thời gian vào của 
                //Khách hàng đang ở hiện tại để tính thời gian
                //Tính thời gian thông qua tổng giây của hiện tại trừ đi tổng giây của giờ vào
                sqlCon.Open();
        ...
        {
            takeInfoCustomer(lblMaPhong1.Text);//khi click đưa thông tin mã phòng và khách lên
            ...
            if (panelPhong1.BackColor == Color.FromArgb(153, 204, 51)) {//đăng ký nhanh thông tin cho khách hàng
            ...
            else
            {
                ...
                {
                    suKienThemDichVu?.Invoke(this, e);//hiện thực sự kiện thêm dịch vụ kích hoạt chuyển trang
                    ServiceNV.MaPhongTextBox = lblMaPhong1.Text;
                }
                if (ThongBaoTInhTien_ThemDichVu.xacNhanChuyenTrang == -1)
                {
                    suKienTinhTien?.Invoke(this, e);
                    
                }
                else return;
            }
            //(panelPhong1.BackColor == Color.FromArgb(255, 128, 0))
        }

        private void panelPhong2_Click(object sender, EventArgs e)
        {
            takeInfoCustomer(lblMaPhong2.Text);//khi click đưa thông tin mã phòng và khách lên
            ...
            else
            {
                ...
                if (ThongBaoTInhTien_ThemDichVu.xacNhanChuyenTrang == 1)
                {
                    suKienThemDichVu?.Invoke(this, e);
        ...
        {
            takeInfoCustomer(lblMaPhong3.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            takeInfoCustomer(lblMaPhong4.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            takeInfoCustomer(lblMaPhong5.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            takeInfoCustomer(lblMaPhong6.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            takeInfoCustomer(lblMaPhong7.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            takeInfoCustomer(lblMaPhong8.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            takeInfoCustomer(lblMaPhong9.Text);//khi click đưa thông tin mã phòng và khách lên
            ...
            {
                //Đưa mã phòng vào ô textbox của form đăng ký xác nhận
        ...
        {
            takeInfoCustomer(lblMaPhong10.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            takeInfoCustomer(lblMaPhong11.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            takeInfoCustomer(lblMaPhong12.Text);//khi click đưa thông tin mã phòng và khách lên
        ...
        {
            cmbTimKiem.SelectedIndex = 0;
            suKienChuyenTrang2?.Invoke(this, e);
        }
        ////////////////////////////////////////////////////////
        public static void takeInfoCustomer(string ID_Phong)//Lấy dữ liệu khách hàng mỗi khi thao tác
        {
            //Lấy mã phòng để truy ra ID_Khách Hàng đang được thao tác
            using(SqlConnection sqlCon = new SqlConnection(TruyVan.strConnection)) {
                sqlCon.Open();
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = $@"
                        select ID_KhachHang, Name, MaPhong
                        from KhachHang as p
                        where p.MaPhong = '{ID_Phong}' AND p.TimeCheckOut IS NULL 
                                   ";
                cmd.Connection = sqlCon;
                SqlDataReader reader = cmd.ExecuteReader();
                while(reader.Read())
                {
                    ...
                    ID_KhachHang = reader.GetString(0);
                    MaPhong = ID_Phong;
                }  
                reader.Close();
            }
        ...
        public static void TongTienKhachHang(string ID_Khachhang)
        {
            using (SqlConnection sqlCon = new SqlConnection(TruyVan.strConnection))
            {
                _TongTien = 0;
                ...
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = $@"
                        SELECT kh.ID_KhachHang, SUM(dv.Price * sddv.SoLuong) AS TongTien
                        FROM KhachHang kh
                        JOIN SuDungDichVu sddv ON kh.ID_KhachHang = sddv.ID_KhachHang
                        JOIN DichVu dv ON sddv.ID_Service = dv.ID_Service
                ...
                if (reader.Read())
                {
                    ...
                    _TongTien = Convert.ToInt32(reader["TongTien"]);

                }
                reader.Close();
                cmd.CommandText = $@"
                        SELECT  SUM(lp.Gia) AS TongTien
                        FROM KhachHang kh
                        JOIN Phong p ON p.ID_Phong = kh.MaPhong
        ...
        private void RefreshTrang_Tick(object sender, EventArgs e)
        {
            ...
            if(xacThucLoadTrang == true)
            {
                ...
                cmbTimKiem.SelectedIndex = 0;//khi quay trờ về trang thì load lại
                return;
            }
        }
        ...
        private void timKiemPhong(string hienThi)//đưa chuỗi mã phòng để tìm kiếm tìm thấy thì hiện 
        {//Không tìm thấy thì visible thành false
        ...
        private void panelPhong8_Paint(object sender, PaintEventArgs e)
        {

        }

        private void lblSoGioPhut12_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut2_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut3_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut4_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut8_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut7_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut6_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut5_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut9_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut11_Click(object sender, EventArgs e)
        {

        }

        private void lblSoGioPhut1_Click(object sender, EventArgs e)
        {
